...
**2026-07-13 — Design-spec correction pass: 3 real mismatches found and fixed against the design README.**

First hands-on run in the emulator surfaced visual differences from `design_handoff_livestock_app/README.md` that the initial scaffold missed:
- **Cow Detail's 5 section rows** were flat list rows (icon + text + chevron, no card). Spec calls for each to be its own tappable card (18dp radius, 2.5px dark-brown border, shadow — same visual weight as `HerdCard`). `SectionMenuRow` now wraps its content in `HerdCard`.
- **Cow Detail and its 5 section screens' header** was the same cream header used everywhere else. Spec explicitly calls for an *inverted* dark-brown-fill/cream-text header on these 6 screens only ("Header as above" in the spec refers back to Cow Detail's dark header). `HerdTopBar` gained a `dark: Boolean` parameter; Cow Detail's title also now reads "टॅग क्रमांक {tag}" per spec instead of the cow's name.
...
**2026-07-13 — Breeding Info fields made editable + farm milk totals + on-device reminders (no server/push involved).**

- **Breeding Info screen**: `lastHeatDate`, `inseminationDate`, and `expectedCalvingDate` existed in the data model since Day 1 but were only ever shown read-only — there was no way to actually enter them. They're now editable `HerdTextField`s with a Save button. Added two new fields: `pregnancyTestDate` and `semenBreed` (the semen/bull-breed used at insemination, distinct from the cow's own breed). "Date the cow gave birth" already existed as `CalvingHistoryEntity.calvingDate` via the existing calving-history bottom sheet — no new field needed there, it just now feeds the reminder logic below. New migration `0003_breeding_extra_fields.sql`.
- **Farm-wide daily milk total**: a new "आजचे दूध नोंदवा" entry point on Home (water-drop icon) opens a hybrid quick-entry sheet — the farmer picks either "एकूण नोंदवा" (one total for the whole herd, a brand new `daily_milk_totals` table/entity, separate from the existing per-cow `milk_records`) or "गाय निवडून नोंदवा" (picks a cow, which still goes through Cow Detail like every other cow action — no shortcut route was added, preserving the existing "5 section screens only ever pushed from Cow Detail" back-stack rule). New migration `0004_daily_milk_totals.sql`.
- **On-device reminders, no server involved**: built on WorkManager (not `AlarmManager`) — a self-rescheduling daily `OneTimeWorkRequest` (`MilkReminderWorker`) needs no `SCHEDULE_EXACT_ALARM` permission and survives reboots via WorkManager's own persistence, at the cost of exact-to-the-minute timing (acceptable for a "log your milk" nudge). Two reminders a day (morning/evening, customizable in a new "सूचना" section on the Dashboard) check purely local Room data — no milk logged today (either farm total or any per-cow record) triggers a notification; already logged, no notification. The morning run additionally checks every cow's breeding cycle: 21 days (the bovine estrus length) since her last recorded heat or last calving — whichever is later — triggers a "check for heat" notification, and it stops once a positive pregnancy test (`pregnancyTestDate` + status "गाभण") is recorded. Added `POST_NOTIFICATIONS` permission + runtime prompt (API 33+); Room bumped to `version = 3` with a real (non-destructive) migration.
- Verified with `./gradlew assembleDebug` and confirming both new migrations applied to `thitdaiznbuxkejvitex` (columns/table visible via `list_tables`, `get_advisors` clean of new issues). Not yet tested on-device: editing/saving all 5 breeding fields, both milk-entry paths, actually receiving a reminder notification at a configured time, and the 21-day heat-check firing/stopping correctly.
